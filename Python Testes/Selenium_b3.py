...
class Webscraping:
    ...
    def Iniciar_sistema(self):
        ...
        self.chrome.find_element_by_xpath(
            '//*[@id="sb-search-open"]/span').click()
        self.chrome.find_element_by_xpath(
            '//*[@id="query"]').send_keys(self.ticker + Keys.ENTER)
        try:
            wait(self.chrome, 3).until(EC.element_to_be_clickable(
    ...
    def Validar_ticker(self):
        ...
        try:
            self.chrome.find_element_by_xpath(
                '//*[@id="richSnippet"]/div[3]/div/div[2]/div[3]/a').click()
    ...
    def Validar_sistema(self):
        try:
            ...
            iframe = self.chrome.find_element_by_xpath(
                '//*[@id="bvmf_iframe"]')
            self.chrome.switch_to.frame(iframe)
            # Aguardo até 10s para carregar o elemento selecionado pelo xpath
            wait(self.chrome, 10).until(EC.element_to_be_clickable(
                (By.XPATH, '//*[@id="ctl00_contentPlaceHolderConteudo_MenuEmpresasListadas1_tabMenuEmpresa_tabRelatoriosFinanceiros"]/span/span'))).click()
            sleep(1.5)
            ...
            print("-" * 40)
            print(
                "\033[1;31m" + "##Sistema indisponível. Tente novamente mais tarde!##   " + "\033[m")
            print("-" * 40 + "\n")
    ...
    def Baixar_planilhas(self):
        ...
        wait(self.chrome, 5).until(EC.element_to_be_clickable(
            (By.XPATH, '//*[@id="ctl00_contentPlaceHolderConteudo_lkbDownload"]'))).click()
        # 31/12/2020 - Informe do Código de Governança - Versão 1.0
        wait(self.chrome, 5).until(EC.element_to_be_clickable(
            (By.XPATH, '//*[@id="ctl00_contentPlaceHolderConteudo_rptDocumentosGovDownload_ctl00_lnkDownload"]'))).click()
        sleep(2.5)
